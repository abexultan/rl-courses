...
class CartPoleSwingUpEnv(gym.Env):
    ...
    Description:
       ...
       a track. Unlike CartPoleEnv, friction is taken into account in the
       physics calculations. The pendulum starts (pointing down) upside down,
       and the goal is to swing it up
...
class CartPoleSwingUpViewer:
    ...
    def _init_cart(self, rendering, cartwidth, cartheight):
        ...
        cart = rendering.FilledPolygon([(lef, bot), (lef, top), (rig, top),
                                        (rig, bot)])
    ...
    def _init_pole(self, rendering, polewidth, polelength):
        ...
        pole = rendering.FilledPolygon([(lef, bot), (lef, top), (rig, top),
                                        (rig, bot)])
    ...
    def update(self, state, pole):
        ...
        self.transforms["pole_bob"].set_translation(
            -pole.length * np.sin(state.theta), pole.length *
            np.cos(state.theta)
        )
