...
class CartPoleEnv(gym.Env):
    ...
    def __init__(self, theta_desired=0, pos_desired=0):
        self.gravity = 9.8
        self.masscart = 1.0
        self.masspole = 0.1
        ...
        self.theta_threshold_radians = 0.05  # 0.1 # 12 * 2 * math.pi / 360
        ...
        high_action_space = np.array([1.0], dtype=np.float32)
...
class CartPoleEnvState(CartPoleEnv):
    ...
    def render(self, mode='human'):
        ...
        x = self.state
        cartx = (self.pos_desired - x[0]) * scale\
            + screen_width / 2.0  # MIDDLE OF CART
        self.carttrans.set_translation(cartx, carty)
        self.poletrans.set_rotation(-x[2])
...
class CartPoleEnvFullStateSwingUp(CartPoleEnvFullState):
...
if __name__ == "__main__":
    ...
    env = CartPoleEnvFullStateSwingUp(mode='train')
    counter = 0
    n_games = 400
    for n in range(n_games):
        ...
        while not done:
            ...
            if (counter + 1) % 700 == 0:
                ...
                print(f'Thete error is = {observation_[2]}')
                counter = 0
